...
var materials = [new _Material(1, [vec4(1,0,0,1), vec4(0,0,0,1), vec4(1,1,1,1), vec4(1,1,1,1), vec4(0,0,0,1), vec4(1, 8, 32, 10), vec4(1,1,0,0)]),
 new _Material(1, [vec4(0,1,0,1), vec4(1,1,1,1), vec4(0,0,0,1), vec4(1,1,1,1), vec4(1,0,0,1), vec4(1, 8, 32, 10), vec4(1,1,0,0)]),
 new _Material(1, [vec4(0,0,1,1), vec4(1,1,1,1), vec4(1,1,1,1), vec4(0,0,0,1), vec4(0,1,1,.5), vec4(1, 8, 32, 10), vec4(1,.1,0,0)]),
 new _Material(1, [vec4(1,1,0,1), vec4(0,0,0,1), vec4(0,0,0,1), vec4(1,1,1,1), vec4(1,1,1,1), vec4(10, 8, 32, 10), vec4(1,1,0,0)]),
 new _Material(1, [vec4(1,0,1,1), vec4(1,1,1,1), vec4(1,1,1,1), vec4(1,1,1,1), vec4(0,0,0,1), vec4(.1, 8, 32, 10), vec4(10,1,0,0)]),
 new _Material(1, [vec4(0,1,1,1), vec4(0,0,0,1), vec4(0,0,0,1), vec4(1,1,1,1), vec4(0,0,0,1), vec4(1, 8, 32, 10), vec4(1,1,0,0)])]
 ...
 /* draw the edges as lines */
 {
   ...
   for (i=0; i<perimeters; i++) {
     if (perimeter[i].draw == true) {
       ...
       walls.push(new _Object({pos: vec3((p2[0]+p1[0])/2, 6, (p2[1]+p1[1])/2), rot: Quaternion(1,0,0,0), scl: vec3(1,1,1)},
       [{pointIndex: tmp.index, matIndex: [0], texCoords: tmp.texCoords, type: _gl.TRIANGLES, normals: tmp.normals, tangents: tmp.tangents, textureIndex: -1}],
       tmp.points, [new _ScaledTexMat(true, .1, .1, 0, 0, 8, 32, .1)], _Bounds._RECT, []))
     }
   }
 }
}
